...
router.put("/user-details/:userName", async (req, res) => {
    ...
    const firstName = (req.body.firstName) ? req.body.firstName : existingUser.firstName
    const lastName = (req.body.lastName) ? req.body.lastName : existingUser.lastName
    const address = (req.body.address) ? req.body.address : existingUser.address
    const phoneNumber = (req.body.phoneNumber) ? req.body.phoneNumber : existingUser.phoneNumber
    const password = (req.body.password) ? await bcrypt.hash(req.body.password, 10) : existingUser.password
